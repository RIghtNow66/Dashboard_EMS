    ...
    m_isEditable(false)
{
    ...
    connect(deleteShortcut, &QShortcut::activated,
        this, &CustomDash::on_deleteWidgetButton_clicked);
}
...
void CustomDash::on_editCanvasButton_clicked()
{
    if (m_isEditable) {
        // 如果当前是可编辑状态，则切换为不可编辑
        m_isEditable = false;
        ui->editCanvasButton->setText("启动编辑");
        setCanvasEditable(false);
    }
    else {
        const QString correctPassword = "scada";
        while (1)
        {
            bool ok;
            // 如果当前是不可编辑状态，则弹出二次确认对话框
            QString password = QInputDialog::getText(this, "密码验证",
                "请输入密码以进入编辑模式:",
                QLineEdit::Password,
                "", &ok);
            if (!ok)
...
void CustomDash::on_newCanvasButton_clicked()
{
    if (!m_isEditable)
    {
        QMessageBox::information(this, "提示", "请先进入编辑模式!");
		return;
    }
    bool ok;
...
void CustomDash::on_canvasList_currentIndexChanged(int index)
{
    ...
    setCanvasEditable(m_isEditable);
}
