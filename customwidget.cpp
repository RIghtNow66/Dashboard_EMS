...
void CustomWidget::showAlignmentGuidelines()
{
    ...
    // 找到距离最近的控件
    CustomWidget* nearestWidget = nullptr;
    ...
    foreach(QGraphicsItem * item, s->items())
    {
        ...
        // 计算两个控件中心点的距离
        QPointF thisCenter = thisRect.center();
        QPointF otherCenter = otherRect.center();
        qreal distance = QLineF(thisCenter, otherCenter).length();
    ...
    // 如果没找到最近的控件，返回
    if (!nearestWidget) return;
    QRectF nearestRect = nearestWidget->sceneBoundingRect();
    QPen guidelinePen(Qt::gray, 1, Qt::DashLine);

    // 只检查与最近控件的6种对齐方式，且只显示在阈值内的

    // 1. 中心X对齐
    if (qAbs(thisCenterX - nearestRect.center().x()) < SNAP_THRESHOLD)
    {
        QGraphicsLineItem* line = new QGraphicsLineItem();
        line->setLine(nearestRect.center().x(), s->sceneRect().top(),
            nearestRect.center().x(), s->sceneRect().bottom());
    ...
    // 2. 中心Y对齐
    if (qAbs(thisCenterY - nearestRect.center().y()) < SNAP_THRESHOLD)
    {
        QGraphicsLineItem* line = new QGraphicsLineItem();
        line->setLine(s->sceneRect().left(), nearestRect.center().y(),
            s->sceneRect().right(), nearestRect.center().y());
        line->setPen(guidelinePen);
        line->setZValue(-1);
        s->addItem(line);
        m_guidelines.append(line);
    ...
    // 3. 左边缘对齐
    if (qAbs(thisLeft - nearestRect.left()) < SNAP_THRESHOLD)
    {
        QGraphicsLineItem* line = new QGraphicsLineItem();
        line->setLine(nearestRect.left(), s->sceneRect().top(),
            nearestRect.left(), s->sceneRect().bottom());
        line->setPen(guidelinePen);
        line->setZValue(-1);
        s->addItem(line);
        m_guidelines.append(line);
    ...
    // 4. 右边缘对齐  
    if (qAbs(thisRight - nearestRect.right()) < SNAP_THRESHOLD)
    {
        QGraphicsLineItem* line = new QGraphicsLineItem();
        line->setLine(nearestRect.right(), s->sceneRect().top(),
            nearestRect.right(), s->sceneRect().bottom());
        line->setPen(guidelinePen);
        line->setZValue(-1);
        s->addItem(line);
        m_guidelines.append(line);
    ...
    // 5. 上边缘对齐
    if (qAbs(thisTop - nearestRect.top()) < SNAP_THRESHOLD)
    {
        QGraphicsLineItem* line = new QGraphicsLineItem();
        line->setLine(s->sceneRect().left(), nearestRect.top(),
            s->sceneRect().right(), nearestRect.top());
        line->setPen(guidelinePen);
        line->setZValue(-1);
        s->addItem(line);
        m_guidelines.append(line);
    ...
    // 6. 下边缘对齐
    if (qAbs(thisBottom - nearestRect.bottom()) < SNAP_THRESHOLD)
    {
        QGraphicsLineItem* line = new QGraphicsLineItem();
        line->setLine(s->sceneRect().left(), nearestRect.bottom(),
            s->sceneRect().right(), nearestRect.bottom());
        line->setPen(guidelinePen);
        line->setZValue(-1);
        s->addItem(line);
        m_guidelines.append(line);
